...
{
    internal class OpenDocumentsSearchResultTreeBuilder
    {
        ...
        public OpenDocumentsSearchResultTreeBuilder()
        {
        }
    }
    ...
    {
        OpenDocumentsSearchResultTreeBuilder treeGroup = new OpenDocumentsSearchResultTreeBuilder();
        ResultData<ISearchStrategy> resultData = await SearchStrategyFactory.Create(Config);
        if (resultData.IsError)
        {
            AppCore.ShowMsg(resultData.Msg);
