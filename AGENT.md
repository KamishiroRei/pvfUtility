...
原始 BAML 的类型表和延迟资源仍引用账号、云备份、商店、ChatGPT 等已停用功能的部分 CLR 类型、枚举和模板属性。这些成员是反序列化兼容契约，不是联网功能入口。不得仅因 C# 中没有直接调用就删除；缺失成员可能直到延迟资源实例化时才以 `WpfXamlLoader.TransformNodes` 空引用的形式失败。

离线功能边界必须在兼容类型之外保持：云 GET/POST 不得创建或调用外网客户端，ChatGPT 不得保留客户端或密钥，更新和遥测程序集不得进入输出，原始 BAML 中遗留的联网控件必须从最终界面移除。修改兼容类型后必须运行 `scripts/Test-RecoveredStartup.ps1`，仅通过编译不算完成验证。
