...
含义：商店类型字符串，主目标可见 `` `[weapon shop]` ``、`` `[etc shop]` `` 等类型。
...
含义：商店页签名称列表。主目标普通装备商店、职业/活动商店等样本可见一个或多个页签名。
...
含义：每日轮换商店的静态开始时间线索。主目标只在每日商品结构样本中观察到。
...
已验证边界：当前主目标实机样本证明，NPC 商店出售 stackable 商品时会读取该字段作为金币价格，并在购买后扣除金币、发放道具。
...
已验证边界：当前主目标实机样本证明，NPC 商店出售 stackable 商品时可按该字段显示并消耗材料、发放目标道具。
